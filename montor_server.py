...
def _wa_interpretar_mensaje_ia(texto):
    ...
    prompt = (
        ...
        "defecto, USD o EUR solo si se menciona explícitamente), categoria (tomá LITERALMENTE la palabra o "
        "frase corta que la persona usó para nombrar el rubro, lugar o marca del gasto — no la traduzcas a "
        "una categoría genérica ni la reclasifiques. Ej: 'gasté 2000 en pasion (estacionamiento)' → categoria "
        "'Pasión', NO 'Entretenimiento'; 'compré en Coto' → categoria 'Coto', NO 'Supermercado'. Solo dejala "
        "vacía si el mensaje realmente no menciona ninguna palabra que sirva de rubro/lugar), descripcion "
        "(detalle breve opcional, vacío si no aporta nada nuevo).\n"
